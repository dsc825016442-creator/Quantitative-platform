...
页面保留演示数据作为首次刷新失败时的降级内容；生产站点通过托管密钥读取 `TUSHARE_TOKEN`，Token 不进入浏览器或 GitHub。

生产数据链路：

1. 每天北京时间 18:30 触发站内刷新任务。
...
各模块是否可用仍取决于 Tushare 账号积分和接口权限。数据扩展顺序见 `docs/data-source-plan.md`。
